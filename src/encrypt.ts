...
async function encryptLink(url: string, password: string): Promise<string> {
 ...
 */
async function decryptLink(payload: string, password: string): Promise<string> {
...
function showResult(el: HTMLElement, html: string, tone: "success" | "error"): void {
  el.hidden = false;
...
function initEncryptForm(): void {
  const form = document.getElementById("encryptForm") as HTMLFormElement | null;
  const urlInput = document.getElementById("encryptUrl") as HTMLInputElement | null;
  const passwordInput = document.getElementById("encryptPassword") as HTMLInputElement | null;
  const result = document.getElementById("cryptoResult");
  if (!form || !urlInput || !passwordInput || !result) return;
